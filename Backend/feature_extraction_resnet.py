...
class FeatureExtractor(object):
    ...
    def load_model(self):
        ...
        :return:
        """
        if self.model_pretrained == 'resnet50':
            print("Loading resnet50 architecture ...")
            model = models.resnet50(pretrained=True)

        elif self.model_pretrained == 'resnet101':
            print("Loading resnet101 architecture ...")
            model = models.resnet101(pretrained=True)

        elif self.model_pretrained == 'resnet152':
            print("Loading resnet152 architecture ...")
            model = models.resnet152(pretrained=True)

        else:
            raise 'Select ResNet model from models'
...
def feature_extraction_resnet(dataset_name, model_pretrained, output_features_path, param):
    ...
    # Extract features
    ResNet_Features, Files, Images = get_features_folder(
        folder=dataset_path,
    ...
    np.save(output_features_path, ResNet_Features)
...
if __name__ == '__main__':
    ...
    PARAMS = ['101']
    MODELS = ['resnet' + param for param in PARAMS]
