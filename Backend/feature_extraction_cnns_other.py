...
class FeatureExtractor(object):
    ...
    def load_model(self):
        """
        Loads CNNs architecture model
        :return:
        """

        if self.model_pretrained == 'alexnet':
            print("Loading alexnet architecture...")
        ...
        elif self.model_pretrained == 'mobilenet_v2':
            print("Loading mobilenet_v2 architecture ...")
            model = models.mobilenet_v2(pretrained=True)

        else:
            raise 'Select CNN model from models'
...
def feature_extraction_cnns_other(dataset_name, model_pretrained, output_features_path, param):
    ...
    # Extract features
    CNN_Features, Files, Images = get_features_folder(
        folder=dataset_path,
    ...
    np.save(output_features_path, CNN_Features)
    print("All " + ALGORITHM + param + " computation took", "--- %s seconds ---" % (time.time() - START_TIME))


if __name__ == '__main__':

    # Parameters: architectures of ResNet network
    MODELS = ['alexnet', 'vgg16', 'mobilenet_v2']
    ...
    for DATASET_NAME in DATASETS:
        for MODEL in MODELS:
            ...
            OUTPUT_FEATURES_FILE = 'features_' + ALGORITHM + MODEL + DATASET_NAME + EXTENSION
            OUTPUT_FEATURES_PATH = os.path.join(OUTPUT_FEATURES_DIRECTORY, OUTPUT_FEATURES_FILE)

            feature_extraction_cnns_other(
